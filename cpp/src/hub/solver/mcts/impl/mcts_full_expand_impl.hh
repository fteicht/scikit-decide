...
struct FullExpand<Tsolver>::ExpandActionImplementation {
    template <typename Ttransition_mode = typename Tsolver::TransitionMode, typename Enable = void>
    struct Impl {};

    template <typename Ttransition_mode>
    struct Impl <Ttransition_mode,
                 typename std::enable_if<std::is_same<Ttransition_mode, DistributionTransitionMode<Tsolver>>::value>::type> : public ExpandActionImplementation {
    ...
    struct Impl<Ttransition_mode,
                typename std::enable_if<std::is_same<Ttransition_mode, StepTransitionMode<Tsolver>>::value ||
                                        std::is_same<Ttransition_mode, SampleTransitionMode<Tsolver>>::value>::type> : public ExpandActionImplementation {
...
SK_MCTS_FULL_EXPAND_CLASS::FullExpand(const HeuristicFunctor& heuristic)
    : _heuristic(heuristic) {
    _action_expander = std::make_unique<typename ExpandActionImplementation::template Impl<>>(_heuristic);
}
...
SK_MCTS_FULL_EXPAND_CLASS::expand_action(Tsolver& solver,
    ...
    return static_cast<typename ExpandActionImplementation::template Impl<>&>(
                *_action_expander).expand_action(solver, thread_id, state, action);
}
