...
#define SK_RIW_DOMAIN_STATE_HASH_CLASS \
...
SK_RIW_DOMAIN_STATE_HASH_TEMPLATE_DECL
std::size_t SK_RIW_DOMAIN_STATE_HASH_CLASS::Hash::operator()(const Key& k) const {
    return typename Tdomain::State::Hash()(k);
}

SK_RIW_DOMAIN_STATE_HASH_TEMPLATE_DECL
bool SK_RIW_DOMAIN_STATE_HASH_CLASS::Equal::operator()(const Key& k1, const Key& k2) const {
...
SK_RIW_STATE_FEATURE_HASH_TEMPLATE_DECL
std::size_t SK_RIW_STATE_FEATURE_HASH_CLASS::Hash::operator()(const Key& k) const {
    std::size_t seed = 0;
...
SK_RIW_STATE_FEATURE_HASH_TEMPLATE_DECL
bool SK_RIW_STATE_FEATURE_HASH_CLASS::Equal::operator()(const Key& k1, const Key& k2) const {
...
#define SK_RIW_ENVIRONMENT_ROLLOUT_TEMPLATE_DECL \
template <typename Tdomain>

#define SK_RIW_ENVIRONMENT_ROLLOUT_CLASS \
EnvironmentRollout<Tdomain>

SK_RIW_ENVIRONMENT_ROLLOUT_TEMPLATE_DECL
void SK_RIW_ENVIRONMENT_ROLLOUT_CLASS::init_rollout(Tdomain& domain, const std::size_t* thread_id) {
...
SK_RIW_ENVIRONMENT_ROLLOUT_CLASS::progress(Tdomain& domain,
...
SK_RIW_ENVIRONMENT_ROLLOUT_TEMPLATE_DECL
void SK_RIW_ENVIRONMENT_ROLLOUT_CLASS::advance(Tdomain& domain,
                                               const typename Tdomain::State& state,
                                               const typename Tdomain::Action& action,
                                               bool record_action,
                                               const std::size_t* thread_id) {
...
SK_RIW_ENVIRONMENT_ROLLOUT_TEMPLATE_DECL
std::list<typename Tdomain::Action> SK_RIW_ENVIRONMENT_ROLLOUT_CLASS::action_prefix() const {
    return _action_prefix;
}

// === SimulationRollout implementation ===

#define SK_RIW_SIMULATION_ROLLOUT_TEMPLATE_DECL \
template <typename Tdomain>

#define SK_RIW_SIMULATION_ROLLOUT_CLASS \
SimulationRollout<Tdomain>

SK_RIW_SIMULATION_ROLLOUT_TEMPLATE_DECL
void SK_RIW_SIMULATION_ROLLOUT_CLASS::init_rollout([[maybe_unused]] Tdomain& domain,
                                                   [[maybe_unused]] const std::size_t* thread_id) {}

SK_RIW_SIMULATION_ROLLOUT_TEMPLATE_DECL
typename Tdomain::EnvironmentOutcome
SK_RIW_SIMULATION_ROLLOUT_CLASS::progress(Tdomain& domain,
                                          const typename Tdomain::State& state,
                                          const typename Tdomain::Action& action,
                                          const std::size_t* thread_id) {
...
SK_RIW_SIMULATION_ROLLOUT_TEMPLATE_DECL
void SK_RIW_SIMULATION_ROLLOUT_CLASS::advance([[maybe_unused]] Tdomain& domain,
...
SK_RIW_SIMULATION_ROLLOUT_TEMPLATE_DECL
std::list<typename Tdomain::Action> SK_RIW_SIMULATION_ROLLOUT_CLASS::action_prefix() const {
...
RIWSolver<Tdomain, Tfeature_vector, Thashing_policy, Trollout_policy, Texecution_policy>

SK_RIW_SOLVER_TEMPLATE_DECL
SK_RIW_SOLVER_CLASS::RIWSolver(Domain& domain,
...
: _domain(domain), _state_features(state_features),
    ...
    _nb_rollouts(0), _debug_logs(debug_logs), _watchdog(watchdog) {
...
void SK_RIW_SOLVER_CLASS::solve(const State& s) {
    try {
        Logger::info("Running " + ExecutionPolicy::print_type() + " RIW solver from state " + s.print());
        ...
        for (atomic_size_t w = 1 ; w <= nb_of_binary_features ; w++) {
            if(WidthSolver(*this, _domain, _state_features,
                           ...
                           _epsilons_protect, _debug_logs, _watchdog).solve(
                               s, start_time, _nb_rollouts, feature_tuples)) {
                found_solution = true;
                break;
            }
        }
        ...
        Logger::info("RIW " + solution_str + " from state " + s.print() +
        ...
        Logger::error("RIW failed solving from state " + s.print() + ". Reason: " + e.what());
        throw;
    }
}
...
SK_RIW_SOLVER_TEMPLATE_DECL
typename SK_RIW_SOLVER_CLASS::Action SK_RIW_SOLVER_CLASS::get_best_action(const State& s) {
    auto si = _graph.find(Node(s, _domain, _state_features, nullptr));
    ...
    if (next_node == nullptr) {
        Logger::error("SKDECIDE exception: best action's next node from state " + s.print() + " not found in the graph");
        throw std::runtime_error("SKDECIDE exception: best action's next node from state " + s.print() + " not found in the graph");
    }
    if (_debug_logs) { Logger::debug("Expected outcome of best action " + si->best_action->print() +
...
SK_RIW_SOLVER_TEMPLATE_DECL
std::pair<std::size_t, std::size_t> SK_RIW_SOLVER_CLASS::get_exploration_statistics() const {
    std::size_t pruned = 0;
...
SK_RIW_SOLVER_TEMPLATE_DECL
std::list<typename SK_RIW_SOLVER_CLASS::Action> SK_RIW_SOLVER_CLASS::action_prefix() const {
...
SK_RIW_SOLVER_TEMPLATE_DECL
void SK_RIW_SOLVER_CLASS::compute_reachable_subgraph(Node& node, std::unordered_set<Node*>& subgraph) {
...
SK_RIW_SOLVER_TEMPLATE_DECL
void SK_RIW_SOLVER_CLASS::update_graph(std::unordered_set<Node*>& root_subgraph, std::unordered_set<Node*>& child_subgraph) {
...
struct SK_RIW_SOLVER_CLASS::UpdateFrontierImplementation<
            TTexecution_policy,
            typename std::enable_if<std::is_same<TTexecution_policy, SequentialExecution>::value>::type> {
    static void update_frontier(TTexecution_policy& execution_policy,
        ...
        for (auto& p : n->parents) {
            ...
            for (auto& nn : p->children) {
                ...
                if (std::get<2>(nn)) {
                    double tentative_value = std::get<1>(nn) + (discount * std::get<2>(nn)->value);
...
struct SK_RIW_SOLVER_CLASS::UpdateFrontierImplementation<
            TTexecution_policy,
            typename std::enable_if<std::is_same<TTexecution_policy, ParallelExecution>::value>::type> {
    static void update_frontier(TTexecution_policy& execution_policy,
                                const double& discount,
                                std::unordered_set<Node*>& new_frontier, Node* n) {
        std::list<Node*> parents;
        execution_policy.protect([&n, &parents](){
        ...
        for (auto& p : parents) {
            ...
            p->value = -std::numeric_limits<double>::max();
            p->best_action = nullptr;
            execution_policy.protect([&discount, &p](){
                for (auto& nn : p->children) {
                    p->solved = p->solved && std::get<2>(nn) && std::get<2>(nn)->solved;
                    if (std::get<2>(nn)) {
                        double tentative_value = std::get<1>(nn) + (discount * std::get<2>(nn)->value);
...
SK_RIW_SOLVER_TEMPLATE_DECL
void SK_RIW_SOLVER_CLASS::update_frontier(std::unordered_set<Node*>& new_frontier, Node* n) {
    UpdateFrontierImplementation<ExecutionPolicy>::update_frontier(_execution_policy, _discount, new_frontier, n);
}

// === RIWSolver::Node implementation ===

SK_RIW_SOLVER_TEMPLATE_DECL
SK_RIW_SOLVER_CLASS::Node::Node(const State& s, Domain& d,
                                const std::function<std::unique_ptr<FeatureVector> (Domain&, const State&, const std::size_t*)>& state_features,
                                const std::size_t* thread_id)
    : state(s),
...
SK_RIW_SOLVER_TEMPLATE_DECL
SK_RIW_SOLVER_CLASS::WidthSolver::WidthSolver(RIWSolver& parent_solver, Domain& domain,
...
: _parent_solver(parent_solver), _domain(domain), _state_features(state_features),
    ...
    _max_depth(max_depth), _exploration(exploration),
    _epsilon_moving_average_window(epsilon_moving_average_window), _epsilon(epsilon),
    _epsilon_moving_average(epsilon_moving_average), _epsilons(epsilons),
    ...
    _time_mutex(time_mutex), _epsilons_protect(epsilons_protect),
...
bool SK_RIW_SOLVER_CLASS::WidthSolver::solve(const State& s,
                                             ...
                                             TupleVector& feature_tuples) {
    try {
        Logger::info("Running " + ExecutionPolicy::print_type() + " RIW(" + StringConverter::from(_width) + ") solver from state " + s.print());
        ...
        if (_debug_logs) Logger::debug("time budget: " + StringConverter::from(elapsed_time(start_time)) +
        ...
        if (!_watchdog(etime = elapsed_time(start_time), nb_rollouts,
            ...
            states_pruned) {
            Logger::info("RIW(" + StringConverter::from(_width) + ") could not find a solution from state " + s.print());
            return false;
        } else {
            auto end_time = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - local_start_time).count();
            Logger::info("RIW(" + StringConverter::from(_width) + ") finished to solve from state " + s.print() + " in " + StringConverter::from((double) duration / (double) 1e9) + " seconds.");
            return true;
        }
    } catch (const std::exception& e) {
        Logger::error("RIW(" + StringConverter::from(_width) + ") failed solving from state " + s.print() + ". Reason: " + e.what());
        throw;
    }
}

SK_RIW_SOLVER_TEMPLATE_DECL
void SK_RIW_SOLVER_CLASS::WidthSolver::rollout(Node& root_node, TupleVector& feature_tuples, atomic_size_t& nb_rollouts,
    ...
    if (_debug_logs) Logger::debug("New rollout" + ExecutionPolicy::print_thread() +
    ...
    while (!(current_node->solved) && !break_loop) {
        ...
        _execution_policy.protect([this, &current_node, &unsolved_children, &probabilities, &thread_id](){
            if (_debug_logs) Logger::debug("Current state" + ExecutionPolicy::print_thread() + ": " +
        ...
        if (fill_child_node(current_node, pick, new_node, thread_id)) { // terminal state
            if (_debug_logs) {
                _execution_policy.protect([&current_node](){
                    Logger::debug("Found" + ExecutionPolicy::print_thread() +
            ...
            if (_debug_logs) {
                _execution_policy.protect([&current_node](){
                    Logger::debug("Pruning" + ExecutionPolicy::print_thread() +
            ...
            if (_debug_logs) {
                _execution_policy.protect([&current_node](){
                    Logger::debug("Max depth reached" + ExecutionPolicy::print_thread() +
            ...
            if (_debug_logs) {
                _execution_policy.protect([&current_node](){
                    Logger::debug("Time budget consumed" + ExecutionPolicy::print_thread() +
                                  " in state: " + current_node->state.print() +
                                  ", depth=" + StringConverter::from(current_node->depth) +
                                  ", value=" + StringConverter::from(current_node->value) +
                                    ExecutionPolicy::print_thread());
...
SK_RIW_SOLVER_TEMPLATE_DECL
bool SK_RIW_SOLVER_CLASS::WidthSolver::novelty(TupleVector& feature_tuples, Node& n, bool nn) const {
    ...
    n.novelty = nov;
    if (_debug_logs) Logger::debug("Novelty: " + StringConverter::from(nov));
    if (_debug_logs) Logger::debug("Novelty depth check: " + StringConverter::from(novel_depth));
    return novel_depth;
}

SK_RIW_SOLVER_TEMPLATE_DECL
void SK_RIW_SOLVER_CLASS::WidthSolver::generate_tuples(const std::size_t& k,
                                                       const std::size_t& n,
...
SK_RIW_SOLVER_TEMPLATE_DECL
bool SK_RIW_SOLVER_CLASS::WidthSolver::fill_child_node(Node*& node, std::size_t action_number,
                                                       bool& new_node, const std::size_t* thread_id) {
    Node* node_child = nullptr;

    _execution_policy.protect([this, &node, &node_child, &action_number](){
        if (_debug_logs) Logger::debug("Applying " + ExecutionPolicy::print_thread() +
    ...
    if (!node_child) { // first visit
        ...
        _execution_policy.protect([this, &node, &next_node, &new_node, &outcome](){
            ...
            if (new_node) {
                if (_debug_logs) Logger::debug("Exploring" + ExecutionPolicy::print_thread() +
