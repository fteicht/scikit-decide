...
TEST_CASE("Implicit space", "[implicit-space]") {
  skdecide::ImplicitSpace<double> is(
      [](const double &e) { return e > 2.0 && e < 3.0; });
...
TEST_CASE("Enumerable space", "[enumerable-space]") {
  struct TestEnumerableSpace : public skdecide::EnumerableSpace<char> {
    ...
    TestEnumerableSpace(std::initializer_list<char> init) : elements(init) {}
...
TEST_CASE("Serializable space", "[serializable-space]") {
  struct TestSerializableSpace
      : public skdecide::SerializableSpace<std::vector<bool>>,
  ...
  REQUIRE(tss.find(tss.to_jsonable({{false, true}, {true, false}})[0]
                       .get<std::vector<bool>>()) != tss.end());
  REQUIRE(tss.from_jsonable({{true, true}, {false, true}, {true, false}}) ==
          tss);
}
...
TEST_CASE("Implicit distribution", "[implicit-distribution]") {
  ...
  skdecide::ImplicitDistribution<double> id(
      [&d, &g]() -> double { return d(g); });
  std::vector<double> v;
  std::generate_n(std::back_inserter(v), 10, [&id]() { return id.sample(); });
  REQUIRE(std::all_of(v.begin(), v.end(), [](const auto &e) -> bool {
    return e >= 1.0 && e <= 2.0;
  }));
}

TEST_CASE("Discrete distribution", "[discrete-distribution]") {
  skdecide::DiscreteDistribution<std::string> dd = {
      {"one", 0.2},
      {"two", 0.4},
      {"three", 0.4},
      {"two", 0.2}}; // duplicates and non-normalized weights
  std::vector<std::string> v;
  std::generate_n(std::back_inserter(v), 10, [&dd]() { return dd.sample(); });
  REQUIRE(dd.get_values().at("two") == Approx(0.5));
  REQUIRE(std::accumulate(dd.get_values().begin(), dd.get_values().end(), 0.0,
                          [](const auto &a, const auto &e) {
                            return a + e.second;
                          }) == Approx(1.0));
  REQUIRE(std::all_of(v.begin(), v.end(), [&dd](const auto &e) -> bool {
    return dd.get_values().find(e) != dd.get_values().end();
  }));
}
...
TEST_CASE("Single-Value distribution", "[single-value-distribution]") {
  ...
  REQUIRE((svd.get_values().size() == 1 &&
           svd.get_values().at("unique") == Approx(1.0)));
  REQUIRE(std::all_of(v.begin(), v.end(),
                      [](const auto &e) { return e == "unique"; }));
}
