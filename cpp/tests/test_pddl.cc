...
TEST_CASE("PDDL", "[pddl]") {
    for (auto& p : std::filesystem::directory_iterator(std::string(SKDECIDE_SOURCE_DIR) + "/tests/data/pddl")) {
        for (auto& pp : std::filesystem::directory_iterator(p.path() / "domains")) {
            ...
            if (std::filesystem::exists(pp.path() / "domains") &&
                ...
                for (auto& domain : std::filesystem::directory_iterator(pp.path() / "domains")) {
                    std::string instance_number = domain.path().filename().string().substr(
                                                        domain.path().filename().string().find_first_of('-'));
                    auto problem = pp.path() / "instances" / (std::string("instance") + instance_number);
                    skdecide::pddl::PDDL pddl;
                    REQUIRE_NOTHROW( pddl.load({domain.path().string(), problem.string()}) );
                }
                ...
                for (auto& ppp : std::filesystem::directory_iterator(pp.path() / "instances")) {
                    if ((pp.path().filename().string() == "woodworking-sequential-multi-core" ||
                         pp.path().filename().string() == "woodworking-sequential-satisficing") &&
                        ppp.path().filename().string() == "instance-10.pddl") { // bugged empty object declaration on Line 25
                        continue;
                    }
