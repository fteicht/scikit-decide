...
TEST_CASE("Event domain", "[event-domain]") {
    ...
    AlphabetSpace(bool with_capitals) {
      ...
      if (with_capitals) {
        ...
        std::copy(v.begin(), v.end(),
                  std::inserter(_alphabet, _alphabet.end()));
      }
    }
  ...
  class TestEventDomain
      : public skdecide::EventDomain<int, char, AlphabetSpace, AlphabetSpace,
                                     skdecide::ImplicitSpace<char>,
                                     skdecide::ImplicitSpace<char>> {
  public:
    virtual std::unique_ptr<skdecide::ImplicitSpace<char>>
    get_enabled_events(const skdecide::Memory<int> &memory) {
