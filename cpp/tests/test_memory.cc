...
TEST_CASE("History Domain", "[history-domain]") {
  class TestHistoryDomain : public skdecide::HistoryDomain<TestHistoryDomain> {
  public:
    typedef int RawState;
    ...
    std::size_t get_memory_length() const { return this->_memory->size(); }

  private:
    inline virtual std::size_t _get_memory_maxlen() { return 50; }
  };
...
TEST_CASE("Finite History Domain", "[finite-history-domain]") {
  ...
  private:
    inline virtual std::size_t make_memory_maxlen() { return 5; }
  };

  TestFiniteHistoryDomain tfhd;
  REQUIRE(tfhd.check_memory() == true);
  skdecide::Memory<char> m(tfhd.get_memory().begin(), tfhd.get_memory().end(),
                           3);
  REQUIRE(tfhd.check_memory(m) == false);
}

TEST_CASE("Markovian Domain", "[markovian-domain]") {
  class TestUninitializedMarkovianDomain
      : public skdecide::MarkovianDomain<int> {
  public:
